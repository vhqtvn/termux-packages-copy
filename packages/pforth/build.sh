...
termux_step_pre_configure() {
	for file in pfdicdat.h pforth; do
		cp $TERMUX_PKG_HOSTBUILD_DIR/platforms/unix/$file $TERMUX_PKG_SRCDIR/platforms/unix/$file
		touch -d "next hour" $TERMUX_PKG_SRCDIR/platforms/unix/$file
	done

	export TERMUX_PKG_BUILDDIR=$TERMUX_PKG_SRCDIR/platforms/unix
